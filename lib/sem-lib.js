 ...
 */
function positiveIntegerValueOf(num, _default) {
...
function Semaphore(capacity, isFull, priority) {
    ...
    this.priority = utils.isNumeric(priority) ? parseInt(priority, 10) : 15;
}
 ...
 * @return {Interger} Semaphore id
 */
Semaphore.prototype.getId = function getId() {
    return this.id;
};
...
Semaphore.prototype.semGive = function semGive(num) {
    ...
    num = positiveIntegerValueOf(num, 1);
    if (num < 1) {
        num = 1;
    }
    this._numTokens += num;
 ...
 * </ul>
 * @return {Object|false}          item.addCounter(n = 1) => wait for n more token
 */
Semaphore.prototype.semTake = function semTake(settings, result) {
    var self = this,
        item, task, priority, num, timeOut, onTimeOut, group, res;
    ...
    num = positiveIntegerValueOf(num, 1);
    priority = positiveIntegerValueOf(priority, this.priority);
    ...
    res.addCounter = function addCounter(num) {
        item.num += positiveIntegerValueOf(num, 1);
        // utils.consoleWrite('semTake', self.id);
    };
...
Semaphore.prototype._semTake = function _semTake() {
    ...
    // A way to loop through in waiting tasks without blocking the process until done
 ...
 */
Semaphore.prototype.destroy = function(safe) {
    if (this._destroying) {
        return false;
    }
...
Semaphore.prototype._destroy = function() {
    ...
    if (this._checkKeepAlive()) {
        this.emit('error', 'Destroy failed');
        return false;
    }
...
Semaphore.prototype.hasInWaitingTask = function hasInWaitingTask() {
    return this.hasOwnProperty('queue') && this._hasInWaitingTask();
};
...
function semCreateWithWrapping(_Semaphore) {
