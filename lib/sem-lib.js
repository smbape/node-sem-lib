...
_Semaphore.prototype.semGive = function semGive(num) {
    ...
    // utils.consoleWrite('semGive', 'num=' + num, 'id=' + this.id);
    this._semTake();
};
...
_Semaphore.prototype.semFlush = function semFlush() {
    ...
    // utils.consoleWrite('semFlush', 'id=' + this.id);
    this._semTake();
};
...
_Semaphore.prototype.semTake = function semTake(settings, result) {
    ...
    // utils.consoleWrite('semTake', 'item.id=' + item.id, 'id=' + this.id);

    var group = this.queue.get({
        priority: item.priority
    });
    if (!group) {
        group = _getGroup(item);
    }
    group.stack.add(item);
    item.group = group;
    if (utils.isNumeric(timeOut) && timeOut > 0) {
        item.timer = setTimeout(function() {
            // utils.consoleWrite('timeOut', 'id=' + item.semaphore.id);
            _removeItem(item);
...
_Semaphore.prototype._semTake = function _semTake() {
    if (!this.hasInWaitingTask()) {
        // utils.consoleWrite('_semTake not hasInWaitingTask', 'id=' + this.id);
        return;
    }
    ...
    // utils.consoleWrite('_semTake', 'item.id=' + item.id, 'id=' + this.id);
    ...
    if (this._checkKeepAlive()) {
        // utils.consoleWrite('_semTake again', 'id=' + this.id);
        this._semTake();
    }
};

_Semaphore.prototype._checkKeepAlive = function _checkKeepAlive() {
    // utils.consoleWrite('_checkKeepAlive', this.queue.length);
    if (!this.hasInWaitingTask()) {
...
function _removeItem(item) {
    // utils.consoleWrite('_removeItem', 'group.priority=' + item.group.priority, 'group.length=' + item.group.stack.length, 'queue.length=' + item.semaphore.queue.length, 'item.id=' + item.id, 'id=' + item.semaphore.id);
    ...
    if (item.group.stack.length === 0) {
        ...
        // utils.consoleWrite('_removeItem remove group', 'group.priority=' + item.group.priority, 'queue.length=' + item.semaphore.queue.length);
...
function _getGroup(item) {
    // utils.consoleWrite('_getGroup', 'queue.length=' + item.semaphore.queue.length, 'item.id=' + item.id, 'id=' + item.semaphore.id);
    ...
    // utils.consoleWrite('_getGroup add group', 'group.priority=' + group.priority, 'queue.length=' + item.semaphore.queue.length);
    return group;
}
